...
export default function ListaUcenika() {
    ...
    useEffect(() => {
        const fetchUcenici = async () => {
            ...
            const { data: uceniciData, error: uceniciError } = await supabase.from('ucenici').select('*').eq('nastavnik_uuid', user.id);
            if (uceniciError) {
    ...
    const columns = [
        ...
        // { field: 'nastavnik_ime', headerName: 'Klasa Prof: ', flex: 1 },
